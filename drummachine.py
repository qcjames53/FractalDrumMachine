...
class PanZoomCanvas(Gtk.DrawingArea):
    def __init__(self, coord_label, measure_label):
        ...
        self.last_mouse = (0, 0)
        self.zoom = 0.008
        self.offset = [CANVAS_SIZE / 2 - (800 / 2) / self.zoom, CANVAS_SIZE / 2 - (600 / 2) / self.zoom]  # Pan offset in world coords
        self.measure_coords = []
        self.notes = []
        self.drawn_notes = []
    ...
    def on_draw(self, widget, cr):
        ...
        x0 = 0
        y0 = 0
        for i, note in enumerate(self.drawn_notes):
    ...
    def update_measure_label(self):
        if not self.measure_coords:
            self.measure_label.set_text("")
            return
        self.measure_label.set_text(f"({self.measure_coords[0]}, {self.measure_coords[1]})  Notes: {self.notes}")
    ...
    def on_mouse_move(self, widget, event):
        ...
        # Update notes if right clicking
            ...
            notes = self.note_list_to_string(self.determine_notes_from_coords(wx, wy))
            self.measure_coords = [int(wx), int(wy)]
            self.notes = notes
            self.drawn_notes = self.determine_notes_from_coords(wx, wy)
        ...
        # Get the notes from the world coordinates
        notes = self.note_list_to_string(self.determine_notes_from_coords(wx, wy))

        self.coord_label.set_text(f"World: ({int(wx)}, {int(wy)})  Notes: {notes} Zoom: {self.zoom}")
        return True
    ...
    def on_scroll(self, widget, event):
        ...
        if event.direction == Gdk.ScrollDirection.UP:
            ...
            new_zoom = self.zoom
        new_zoom = max(0.001, min(60, new_zoom))
